        ...
        fields: [
          ...
          // ---- Recipes -------------------------------------------------
          {
            ...
            fields: [
              ...
              { type: 'string', name: 'serves', label: 'Serves (optional, e.g. “Serves 4”)' },
              { type: 'string', name: 'time', label: 'Time (optional, e.g. “30 min”)' },
              {
                type: 'object',
                name: 'ingredients',
                label: 'Ingredients (one per line)',
                list: true,
                ui: { itemProps: (i) => ({ label: i?.text || 'Ingredient' }) },
                fields: [{ type: 'string', name: 'text', label: 'Ingredient' }],
              },
              {
                type: 'object',
                name: 'steps',
                label: 'Method (one step per item)',
                list: true,
                ui: { itemProps: (i) => ({ label: i?.text?.slice(0, 40) || 'Step' }) },
                fields: [{ type: 'string', name: 'text', label: 'Step', ui: { component: 'textarea' } }],
              },
              {
                type: 'string',
                name: 'href',
                label: 'External link (optional — set this to make the card open another site instead of its recipe page)',
              },
            ],
          },
